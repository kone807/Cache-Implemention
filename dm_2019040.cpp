...
void DMsearch(Block main_memory[], int mlines, Set cache_memory[], int clines, int address, int n, int num_words)
{
...
void DMwrite(Block main_memory[], int mlines, Set cache_memory[], int num_sets, int address, int value, int n, int num_words)
{
    int offset = address%num_words;
    int block_number = address/num_words;
...
void printCacheMemory(Set cache_memory[], int clines, int num_sets, int num_words)
{
    ...
    cout<<"|"<<"Line| "<<"Block"<<"|"<<"                               Address => Value Pairs (hex => dec)                           ";
    ...
    {   
        ...
        cout<<"|"<<setfill('0')<<setw(4)<<line_number++<<"| "<<setfill('0')<<setw(4)<<cache_memory[i].input[k].id;
...
int main()
{
  ...
  //  printMainMemory(main_memory, mlines, num_words);
    ...
    if(instruction.substr(0,4) == "read")
    {
        ...
        DMsearch(main_memory, mlines, cache_memory, clines, address, n, num_words);
   ...
   else if(instruction.substr(0,5) == "write")
   {
       ...
       DMwrite(main_memory, mlines, cache_memory, clines, address, value, n, num_words);
